...
-   Alternatively, both the client ID and secret can be supplied as environment variables: `OAUTH_CLIENTID` and `OAUTH_SECRET`. If these environment variables are available (and no values are entered as function arguments), they will be used to store credentials. This approach allows for greater flexibility in managing credentials, particularly in environments where security or automation concerns may warrant the use of environment variables for configuration.
...
# This code retrieves environment variables and passes them to the 
# store_cdse_credentials function, which then saves your credentials 
# as a JSON file named cdse_credentials.json.
```
