---
title: "rOPTRAM"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{rOPTRAM}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

# Introduction

### Prepare soil moisture maps covering an area of interest (AOI), over a date range.


`rOPTRAM` implements The OPtical TRapezoid Model (OPTRAM) to derive soil moisture based on the linear relation
  between a vegetation index, i.e. NDVI, and Land Surface Temperature (LST).
  
  The Short Wave Infra-red (SWIR) band is used as a proxy for LST. The SWIR band is transformed to Swir Transformed Reflectance (STR).
  
  A scatterplot of NDVI vs. STR is used to produce wet and dry linear regression lines, and the slope/intercept coefficents of these lines comprise the trapezoid. These coefficients are then used on a new satellite image to determine soil moisture.

  See:  @sadeghi_optical_2017, @burdun_satellite_2020, @ambrosone_retrieving_2020


```{r setup}
remotes::install_gitlab("rsl-bidr/roptram")
library(rOPTRAM)
ls(getNamespace('rOPTRAM'))
# The `sen2r` package is required for downloading Sentinel imagery
install.package("sen2r", dependencies = TRUE)
```

# Main wrapper function

### Run the full OPTRAM model procedure with a single function call

This example uses sets `timeperiod` to "seasonal", so only images between the month of from_date and the month of to_date will be used, but from all years in the date range.

```{r wrapper}
from_date <- "2018-12-01"
to_date <- "2020-04-30"
aoi <- system.file("extdata", "migda_9.gpkg", package = "rOPTRAM")
coeffs <- optram(aoi,
                 from_date, to_date,
                 veg_index = c("SAVI"),
                 timeperiod = "seasonal")
coeffs
```

# Step by step

### The same procedure as the wrapper function, but in explicit steps

  - Acquire Sentinel 2 images within a date range, and crop to AOI;
  - Prepare the SWIR Transformed Reflectance;
  - Prepare a data.frame of Vegetation Index and STR values;
  - Get trapezoid coefficients from the scatterplot of VI-STR pixels

This example uses sets `timeperiod` to "full", so all images between from_date and to_date will be used.

```{r stepbystep}
from_date <- "2018-12-01"
to_date <- "2019-04-30"
aoi <- system.file("extdata", "migda_aoi.gpkg", package = "rOPTRAM")
s2_file_list <- optram_acquire_s2(aoi,
                                 from_date, to_date,
                                 timeperiod = "full",
                                 list_indicies="MSAVI2")
BOA_dir <- system.file("extdata", "BOA", package = "rOPTRAM")
STR_list <- optram_calculate_str(BOA_dir)
VI_list <- list.files(system.file("extdata", "NDVI", package = "rOPTRAM"), pattern = ".tif$", full.names = TRUE)
full_df <- optram_ndvi_str(STR_list, VI_list)
coeffs <- optram_wetdry_coefficients(full_df, aoi_file)
coeffs
```

# Manually downloaded Sentinel images

### This function allows users to:

  - manually download Copernicus Sentinel-2 imagery in advance
  - full control of image dates, cloud cover, etc
  - run the OPTRAM algorithm to prepare soil moisture using those images

```{r s2}
aoi_file <- system.file("extdata", "migda.gpkg", package = "rOPTRAM")
safe_dir  <- "/home/micha/EO_Data/Israel/Migda_09/"
coeffs <- optram_safe(safe_dir, aoi_file)
```

# Soil Moisture

### Using the trapezoid coefficients and VI, STR rasters, prepare a soil moisture grid

```{r soil-moisture}
img_date <- "2023-03-11"
VI_dir <- system.file("extdata", "NDVI", package = "rOPTRAM")
STR_dir <- system_file("extdata", "STR", package = "rOPTRAM")
coeffs_file <- system_file("extdata", "coefficients.csv", package = "rOPTRAM")
SM <- optram_calculate_soil_moisture(img_date, VI_dir, STR_dir, coeffs_file)
```
