...
```r
rmse <- optram(aoi_file,
                 from_date, to_date,
                 S2_output_dir = output_dir,
                 data_output_dir = output_dir)
knitr::kable(rmse, caption = "RMSE for linear fitted trapezoid edges")
=======
...
#> 5: knitr::kable(rmse, caption = "RMSE for linear fitted trapezoid edges")
#> 6: ==
#>    ^
```
### Show trapezoid plot
...
#> Error in file.path(output_dir, "trapezoid_edges_lin.csv"): object 'output_dir' not found
df_file <- file.path(output_dir, "VI_STR_data.rds")
#> Error in file.path(output_dir, "VI_STR_data.rds"): object 'output_dir' not found
full_df <- readRDS(df_file)
#> Error in readRDS(df_file): object 'df_file' not found
plot_vi_str_cloud(full_df, ttl, edges_df, output_dir = output_dir)
#> Error in plot_vi_str_cloud(full_df, ttl, edges_df, output_dir = output_dir): unused argument (output_dir = output_dir)
...
#> Error in check_aoi(aoi): object 'aoi' not found
STR_list <- list.files(file.path(output_dir, "STR"),
                      pattern = ".tif$", full.names = TRUE)
#> Error in file.path(output_dir, "STR"): object 'output_dir' not found
VI_list <- list.files(file.path(output_dir, "NDVI"),
                      pattern = ".tif$", full.names = TRUE)
#> Error in file.path(output_dir, "NDVI"): object 'output_dir' not found
full_df <- optram_ndvi_str(STR_list, VI_list,
                           output_dir = output_dir)
#> Error in optram_ndvi_str(STR_list, VI_list, output_dir = output_dir): object 'STR_list' not found
rmse <- optram_wetdry_coefficients(full_df,
                                   output_dir = output_dir)
#> Error in optram_wetdry_coefficients(full_df, output_dir = output_dir): object 'full_df' not found
...
#> Error in file.path(output_dir, "STR"): object 'output_dir' not found
data_dir <- output_dir
#> Error in eval(expr, envir, enclos): object 'output_dir' not found
SM <- optram_calculate_soil_moisture(img_date, VI_dir, STR_dir,
                                     data_dir = data_dir)
#> Error in dir.exists(VI_dir): object 'VI_dir' not found
```
...
#> Error in names(SM) <- "Lachish soil moisture": object 'SM' not found
SM[terra::values(SM) < 0] <- 0
#> Error in SM[terra::values(SM) < 0] <- 0: object 'SM' not found
...
sm_map <- leaflet("OpenStreetMap") |>
  ...
  addRasterImage(SM,
    ...
    opacity = 0.9)
#> Error in addRasterImage(addTiles(leaflet("OpenStreetMap")), SM, colors = colorNumeric("RdYlGn", : object 'SM' not found
sm_map
