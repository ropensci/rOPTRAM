...
#  meanwhile scaling is set: gain and offset. There is extraction from metadata down
    ...
    # ???????????????????????????????? aoi_to_name
    #                                  Error in aoi_to_name(aoi_file) :
    # could not find function "aoi_to_name"
...
#        xml_file <- list.files(s, pattern = "MTL.*xml$", full.names = TRUE)[1]
...
#        img_stk <- terra::rast(img_10m_list)
        ...
        # the name is formed fron folder
...
#      stk <- derived_rasters[[x]] this isn't working, works [x]
