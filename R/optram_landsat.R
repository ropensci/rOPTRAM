...
#' @export
#' @note Unlike the \code{\link[rOPTRAM]{optram_acquire_s2}} function,
#' there is no implementation for automatic download of Landsat images.
...
#' as well as the metadata files as downloaded from (i.e.) the USGS EarthExplorer:
#'  (\url{https://earthexplorer.usgs.gov/}) website.
#' @examples
#' \dontrun{
...
#' }
  ...
  datestr <- VI_STR_list <- stk <- VI_df <- VI_idx <- STR <- STR_df <- full_df <- NULL
  ...
  LS_list <- grep("L*02*", list.dirs(landsat_dir), value = TRUE)
  LS_b1 <- grep("*_B1.TIF$", list.files(LS_list[1], full.names = TRUE),
                value = TRUE)
...
#' @param landsat_dir, folder, contains the manually downloaded Landsat imagers
